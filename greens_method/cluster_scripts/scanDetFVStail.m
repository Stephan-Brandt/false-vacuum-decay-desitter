...
ellMin=ToExpression[$CommandLine[[-3]]];
sMin = ToExpression[$CommandLine[[-2]]];
sMax = ToExpression[$CommandLine[[-1]]];
...
Op3[f_,ell_,chi_,s_]:=(-3Cot[chi] D[f,chi]-D[D[f,chi],chi])+Sin[chi]^(-2)ell(ell+2)f+ m2\[Phi]EffInterp[Pi]f+s f;
...
For[ss=sMin,ss<sMax+1,ss=ss+125,
...
If[Mod[ss-sMin,500]==0,Print[StringJoin["Computed s=",ToString[ss]]]];
];
Export[StringJoin["greenSparamFV-",ToString[ellMin],"-",ToString[sMin],"-",ToString[sMax],".csv"],reconstructed](* ;]; *)
];
